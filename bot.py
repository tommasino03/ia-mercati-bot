...
def multi_correlazione(indices_values, small_alerts):
    if not small_alerts:
        return None
    trends = {name: (1 if val>0 else -1) for name,val in indices_values.items() if val is not None}
    if not trends:
        return None
    small_trend = [1 if v>0 else -1 for v in small_alerts.values()]
    # Conta small-cap che seguono almeno uno degli indici
    same_trend_counts = {name: sum(1 for t in small_trend if t == trend) for name, trend in trends.items()}
    messages = []
    for name,count in same_trend_counts.items():
        percent = count / len(small_alerts) * 100
        if percent >= 60:
            direzione = "rialzista" if trends[name]>0 else "ribassista"
            messages.append(f"⚡ {name} {direzione} e {int(percent)}% small-cap seguono il trend")
    if messages:
        return "\n".join(messages)
    return None
...
async def main():
    ...
    try:
        ...
        # Multi-correlazione
        corr_msg = multi_correlazione(indices_values, small_alert)
        if corr_msg:
            alert_flag = True
            msg += f"\n{corr_msg}\n"
        ...
        print("Errore durante controllo mercati/notizie/grafico/multi-correlazione:", e)
